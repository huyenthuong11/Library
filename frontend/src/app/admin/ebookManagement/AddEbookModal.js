...
export default function AddEbookModal({ open, handleClose, onSuccess }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            setIsSaving(true);
            await api.post(`/ebooks/create`, formData, { headers: { "Content-Type": "multipart/form-data" }});
            alert("Thêm Ebook thành công!");
            if (onSuccess) onSuccess();
            handleClose();
        } catch (error) { 
            alert("Lỗi khi thêm Ebook"); 
        } finally { 
            setIsSaving(false); 
        }
    };
    ...
    return (
        <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
            <DialogTitle sx={{ fontWeight: "bold", color: "#0b485e", textAlign: "center" }}>Thêm Ebook mới</DialogTitle>
            <DialogContent sx={{ mt: 1, display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                <div style={{ marginTop: "10px" }}>
                    <p style={{ margin: "0 0 5px 0", fontSize: "14px", fontWeight: "bold" }}>Tải ảnh bìa:</p>
                    <input type="file" accept="image/*" onChange={(e) => setCoverImage(e.target.files[0])} />
                </div>
                <div style={{ marginTop: "10px" }}>
                    <p style={{ margin: "0 0 5px 0", fontSize: "14px", fontWeight: "bold" }}>Tải file sách:</p>
            ...
            <DialogActions sx={{ p: 2, justifyContent: "center" }}>
                ...
                <Button onClick={handleSubmit} variant="contained" disabled={isSaving} sx={{ bgcolor: "#0b485e" }}>
